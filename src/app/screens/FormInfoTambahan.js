    ...
    useEffect(() => {
        ...
        const fetchUkuranBaju = async () => {
            ...
            try {
                ...
                const { data, error } = await supabase.from("ukuran_baju").select("*");
                if (error) throw error;
                setBajuOptions(
                    data.map((item) => ({ value: item.ukuran, label: item.ukuran }))
                );
            } catch (error) {
    ...
    return (
        ...
        >
            ...
            <Select
                name="ukuranBaju"
                options={bajuOptions}
                value={bajuOptions.find(option => option.value === form.ukuranBaju)}
            ...
            <label className="block text-sm font-medium text-gray-700 mb-1">Ukuran Celana</label>
            <input
                name="ukuranCelana"
            ...
            <label className="block text-sm font-medium text-gray-700 mb-1">Sepatu Sekolah</label>
            <input
                name="ukuranSepatu"
                type="number"
            ...
            <div className="flex justify-between mt-6 gap-8">
                ...
                <Button label="Selanjutnya" onClick={() => setStep(8)} />
            </div>
        </form>
    );
}